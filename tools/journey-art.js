...
export function createJourneyArt(container,theme){
  ...
  if(theme==='candy'){
    ...
    lollipop(3.8,3,-1,.63,'#a9c9b2',.38);
    lollipop(-4,-.85,-.6,.54,'#d9b571',.3);
    ...
    const cake=group(3.2,-.5,.5,.1);part(new THREE.CylinderGeometry(.55,.55,.55,28),'#e5bca9',[0,0,0],[1,1,1],cake);part(new THREE.CylinderGeometry(.57,.57,.15,28),'#fff3df',[0,.3,0],[1,1,1],cake);ball('#ce857c',[0,.49,0],[.13,.17,.13],cake);
    ...
    const question=group(2.8,2.6,0,-.2);const arc=part(new THREE.TorusGeometry(.5,.12,12,32,Math.PI*1.55),'#9abfb9',[0,0,0],[1,1,1],question);arc.rotation.z=-.45;ball('#9abfb9',[.08,-.95,0],[.13,.13,.13],question);
