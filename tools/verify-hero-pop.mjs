...
try{
  ...
  for(const id of ['candy','moon','question']){
    ...
    await p.locator('.journey-voice').first().click();await p.locator('.journey-voice').nth(1).click();await p.waitForFunction(()=>window.lastJourneyAudio.currentSrc.includes('-2.mp3')&&window.lastJourneyAudio.currentTime>.12&&!window.lastJourneyAudio.paused);await p.waitForSelector('.journey-voice[data-playing=true]');assert.equal(await p.locator('.journey-voice[data-playing=true]').count(),1);
