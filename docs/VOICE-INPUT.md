...
- 临时转写不反复打断读屏，最终转写为 polite 通知。最终原话在思考、朗读和暂停时保留；长句可在三行高度内滚动阅读。转写与状态均通过可选 `sanitize` 处理通过 React 文本节点安全呈现。
...
文字动效统一在 `src/calligraph-bubble.jsx` 的 `createTextMotion` 管理，样式在 `src/text-motion.css`，由 Calligraph 执行逐字淡入、轻微位移及模糊消散。转写增量更新保留已有前缀，只让新增文字入场；清空时取消未完成动画。完整文字通过无障碍标签即时可用。正文和通话气泡复用相同实现。

`/dev` 的 `ReadAlong` 按短句选择当前朗读范围，沿用豆包实际音频时间戳，以加粗和光晕表达进度。每字预留加粗后的宽度，入场和高亮均不改变换行。底部状态为 10px 半透明白字，无背景；转写文字统一深灰。
