...
- 开场固定使用角色模拟器里的“河湾小水獭”，直接复用可复现的真实角色 recipe 与动画；问题从它头顶气泡出现。
...
- 《不见了的回声》共 3 章、6 个适合约 5 岁儿童的直白场景。路线固定为纸上地面、萤火草地、月球表面、海底气泡、巨人口袋和云朵里面；先解释“声音碰到远处又回来就是回声”，再陪迷路的小回声回家。
- 每个场景只显示 1 位主 NPC 和 1 位同伴 NPC。两位角色先一对一交谈，把发生的事情说清楚，再把一个具体问题交给孩子回答。
- 横版舞台直接复用角色模拟器的程序化场景画布，开场默认是最简洁的纸上地面；人物保持在场景近处。场景切换先把圆形画面收小到黑场，再从圆心展开新画面。
- NPC 气泡只显示当前对白。字幕按当前语音时长逐字出现，语音结束时完整显示并自动切换下一句；点击气泡会停止当前句并直接继续自动流程。
- 轻触 NPC 和宠物会触发动作与 Organic 音效；轻触空白场景会让宠物分成多步慢慢走到目标位置。
- 听听贝、萤火线团、远方小信和发光种子直接显示在角落的小背包框里，点开仍可查看来历、用途与已送出状态。
