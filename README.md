...
6. 气泡每页只显示一行黑体文字，柔和逐字入场；读完后只能手动点击继续。
...
- NPC 气泡每页最多一行普通黑体，逐字入场并预留阅读时间。语音播完不会自动翻页，只能轻触气泡或空白场景继续。
- 轻触 NPC 和宠物会触发动作与 Organic 音效；气泡没有下一页时，轻触空白场景会让宠物走到目标位置。
- 听风贝、星线团、没寄出的问候和灯塔种子进入角落背包，显示来历、用途与已送出状态。
- 四套故事发散、世界观、三章脚本与技术分工见 `STORY-BIBLE.md`。

豆包文本理解、豆包 TTS 与豆包 ASR 均已实测可用。Safari 端采用“浏览器中间文本 + 豆包句末校准”的混合体验；孩子第一次点麦克风后保持同一条媒体轨道。角色说话和 AI 理解期间会暂停识别，孩子手动点到下一句或下一个问题后再继续听，避免角色声音被识别成孩子回答。
