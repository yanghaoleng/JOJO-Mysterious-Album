...
接收当前访谈问题、孩子这一轮说的话和可选的 `forceRespond`。豆包先返回 `shouldRespond`、关键词与继续倾听提示；内容完整时才生成回应、低敏感度偏好和宠物特征，语气词、等待语和明显没说完的片段不会推进问题。点选预设答案会携带 `forceRespond=true`，保证立即回应。接口不可用时前端执行同口径的确定性判断。
...
新版故事页将一次发言的 16kHz 单声道 PCM 发送到同源服务端，由服务端通过豆包大模型语音识别 WebSocket 返回最终文本。浏览器看不到 AppID 对应的 Access Token。Safari 支持时先用浏览器中间结果即时显示，发言结束后再用豆包结果校准；失败时仍可点选预设回答。单次音频上限 30 秒，服务端不写入音频文件或统计库。
...
- 正式第一关和角色实验室不录音。新版 `/story-v2` 获得麦克风许可后，会把单次发言送到同源 `/api/asr`，服务端再转给豆包识别；当前实现不落盘、不进入统计，但正式上线前仍需单独说明、家长授权和数据保留策略。语音不可用时仍可点选预设回答。产品不建立儿童账号，匿名统计不保存孩子输入、声音或原始 IP。
...
主页入口和独立地址 `http://localhost:8137/story-v2` 实现《不见了的回声》完整结构；它与“雾灯花园”并列，不替换原来的正式第一关：

- 开场固定使用角色模拟器里的“卷尾小狐狸”小卷，直接复用可复现的真实角色 recipe 与动画，不再用页面图形拼一个替代形象；问题从它头顶气泡出现。
- 访谈区只保留一个麦克风图标和三个预设回答，不提供文本输入或提交按钮。孩子的话显示在麦克风上方的小气泡里，豆包通过 `shouldRespond` 判断是继续听还是自然回应并进入下一问。
- 4 轮访谈认识主题、探索方式、陪伴方式和节奏偏好；`POST /api/story-turn` 同时返回回应时机、关键词、低敏感度偏好和宠物特征提示，无网络时使用确定性映射继续。
...
豆包文本理解、豆包 TTS 与豆包 ASR 均已实测可用。Safari 端采用“浏览器中间文本 + 豆包句末校准”的混合体验；孩子第一次点麦克风后保持同一条媒体轨道，浏览器语音识别结束时自动重启。图鉴员说话和豆包理解期间会暂停识别，话音结束自动继续，避免把角色声音误认成孩子回答。没有提交动作，AI 会过滤“嗯”“我还想一想”等未完成表达，并在听到完整偏好后回应；点选答案仍作为无麦克风降级入口。
