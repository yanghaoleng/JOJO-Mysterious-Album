...
本次新增首页第五个入口《第一束好奇光》，沿用绘本封面卡片，进入故事介绍页 `/wow`；完整互动体验位于 `/wow-story`。完整脚本见 [`docs/WOW-STORYLINE.md`](docs/WOW-STORYLINE.md)。

- 当前包含 6 章、41 幕。第一章从听见声音开始，依次认识好奇手电筒、哇呜星与灰灰雾、唔姆收音机和颜色罐，再逐步进入后续五个世界。
- 孩子可以输入、说话转文字或点选示例表达。`/wow` 的六色点灯是明确标注的本地演示，不调用 AI，也不写入正式故事进度。
- 故事中的 `/api/wow-turn` 提供 AI 简短回应，并把孩子描述的钥匙映射为页面支持的有限形状与颜色；模型不会生成任意代码或完整绘本。服务不可用时会标明本地回应，仍能继续故事。
- 旅程文本与进度保存在当前浏览器本机，完成后可下载本机生成的旅程纪念册。语音通过本站转文字；故事内 AI 回应会通过本站处理输入或转写的文字。当前不提供云同步、AI 整本绘本生成或成长评估。
