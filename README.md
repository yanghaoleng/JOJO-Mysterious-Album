...
首页《第一束好奇光》进入故事介绍页 `/wow`；原版2D互动体验位于 `/wow-story`，也可通过 `/story-v2?story=wow` 进入。完整确认脚本见 [`docs/WOW-STORYLINE.md`](docs/WOW-STORYLINE.md)。

- 原版2D现直接复用《登月计划》的 `story-v2` 引擎、角色模板、原场景库、连续语音、角色入场、头顶气泡、背包和圆形转场。WOW入口只在本页选择原绘本渲染，不修改用户的3D实验偏好；不再加载此前独立自绘的WOW Canvas舞台。
...
- 每章从偏暗的绘本场景和灰灰雾开始，仅在有效表达被接住后逐步恢复亮度与颜色。停顿、不知道如何表达、权限失败及不适合记录的输入不会增加进度，也不会扣除已有颜色；换章重新揭开新的场景。
