...
- 取名、第一句问候和 6 个场景行动全部使用同一条持续语音会话。角色说话、服务端校准和 AI 理解期间自动暂停收音，结束后自动恢复。
- 《不见了的回声》共 3 章、6 个场景，NPC 分别从门、草堆、箱子、倒影、帘子和灯罩出场。NPC 头顶气泡每页最多两行，轻触气泡或空白场景继续翻页。
...
豆包文本理解、豆包 TTS 与豆包 ASR 均已实测可用。Safari 端采用“浏览器中间文本 + 豆包句末校准”的混合体验；孩子第一次点麦克风后保持同一条媒体轨道，浏览器语音识别结束时自动重启。图鉴员、宠物和 NPC 说话以及豆包理解期间会暂停识别，话音结束自动继续，避免角色声音被识别成孩子回答。没有提交动作，AI 会过滤“嗯”“我还想一想”等未完成表达，并在听到完整偏好或行动后回应。
