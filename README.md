...
- 《不见了的回声》共 3 章、6 个适合约 5 岁儿童的直白场景。开场先解释“声音被远处送回来就是回声”，再沿着声音找到灯塔，最后由孩子说一句话把灯塔点亮。
- 每个场景只显示 1 位主 NPC 和 1 位同伴 NPC。两位角色先一对一交谈，把发生的事情说清楚，再把一个具体问题交给孩子回答。
- 横版舞台使用简洁的扁平绘本物件、明确的地平线和统一的大角色比例。远景位于地平线后，角色位于近景；场景切换先把圆形画面收小到黑场，再从圆心展开新画面。
- NPC 气泡只显示当前对白。字幕按当前语音时长逐字出现，语音结束时完整显示并自动切换下一句；点击气泡会停止当前句并直接继续自动流程。
- 轻触 NPC 和宠物会触发动作与 Organic 音效；轻触空白场景会让宠物分成多步慢慢走到目标位置。
- 听听贝、发光小绳、灯塔小信和发光种子直接显示在角落的小背包框里，点开仍可查看来历、用途与已送出状态。
