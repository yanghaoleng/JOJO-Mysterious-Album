...
import { CHARACTER_CATALOG } from './content/characters.js';
export { CHARACTER_CATALOG } from './content/characters.js';
...
export function createCharacter({ type = 'dog', color, scale = 1 } = {}) {
  ...
  function update(time = 0, dt) {
    ...
    const running = weights.run;
    const sprinting = weights.sprint;
    const jumping = weights.jump;
    const floating = weights.float;
    const sleeping = weights.sleep;
    const rolling = weights.roll;
    const dancing = weights.dance;
    const cheering = weights.cheer;
    const falling = weights.fall;
    const gait = Math.sin(time * (sprinting ? 14 : running ? 10.5 : 8.5));
    const hopPhase = Math.max(0, Math.sin(time * 4.4));
    const jumpPhase = Math.max(0, Math.sin(time * 6.3));
    const breath = Math.sin(time * 2.15);
    const floatY = .85 + Math.sin(time * 2.1) * .07;
    const sleepTilt = sleeping * (Math.PI / 2);
    const rollSpin = rolling * (time % 6.283);
    const cheerPhase = Math.max(0, Math.sin(time * 7.2));
    airborneHeight = hopping * hopPhase * .20 * fit.scale.y + jumping * jumpPhase * .55 * fit.scale.y + floating * floatY * fit.scale.y;
    actor.position.y = hopping * hopPhase * .20 + jumping * jumpPhase * .55 + floating * floatY + walking * Math.abs(gait) * .03 + sleeping * -.42 + falling * Math.min(.5, Math.max(0, (time % 1.3))) * .18;
    actor.rotation.z = sleepTilt + rolling * rollSpin + falling * .9 + Math.sin(time * 1.45) * .012 + walking * gait * .035 + dancing * Math.sin(time * 3.1) * .09;
    actor.rotation.x = falling * .5;
    actor.scale.set(1 - hopping * hopPhase * .035 + cheering * cheerPhase * .05, 1 + hopping * hopPhase * .06 + jumping * jumpPhase * .08 + cheering * cheerPhase * .12, 1 - hopping * hopPhase * .025);
    ...
    const armSwing = (walking + running + sprinting) * gait * .47 + dancing * Math.sin(time * 3.1) * .35;
    arms[0].rotation.z = damping(arms[0].rotation.z, -.09 + talking * Math.sin(time * 3.6) * .12 - hopping * .35 + cheering * 2.1 - dancing * .3, dt);
    arms[1].rotation.z = damping(arms[1].rotation.z, .09 + wave * (2.2 + Math.sin(time * 7) * .26) + hopping * .35 - cheering * 2.1 + dancing * .3, dt);
    arms[0].rotation.x = damping(arms[0].rotation.x, armSwing - listening * .14, dt);
    arms[1].rotation.x = damping(arms[1].rotation.x, -armSwing - wave * .3, dt);
    arms[1].position.z = damping(arms[1].position.z, wave * .23, dt);
    const footGait = (walking + running + sprinting) * gait;
    feet[0].rotation.x = damping(feet[0].rotation.x, -footGait * .35, dt);
    feet[1].rotation.x = damping(feet[1].rotation.x, footGait * .35, dt);
    feet[0].position.y = .20 + walking * Math.max(0, gait) * .08 + running * Math.max(0, gait) * .11 + sprinting * Math.max(0, gait) * .14 + floating * .3;
    feet[1].position.y = .20 + walking * Math.max(0, -gait) * .08 + running * Math.max(0, -gait) * .11 + sprinting * Math.max(0, -gait) * .14 + floating * .3;
