...
export function planCreation(raw) {
  ...
  // Keep established story recipes stable when a vocabulary model shares their name.
  // The word game has its own parser for composing R-line models and sentences.
  let kits = CREATION_KITS.filter(kit => !kit.id.startsWith('rword-') && new RegExp(kit.words).test(objectWords)).slice(0, 3);
  if (!kits.length) {
    const nounText = helper ? objectWords.replace(helper.name.replace(/（.*$/, ''), '').replace(new RegExp(helper.id, 'ig'), '') : objectWords;
    kits = CREATION_KITS.filter(kit => kit.id.startsWith('rword-') && kit.words.split('|').some(alias => {
      const escaped = alias.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      return new RegExp(/^[a-z -]+$/i.test(alias) ? `\\b${escaped}\\b` : escaped, 'i').test(nounText);
    })).slice(0, 3);
  }
