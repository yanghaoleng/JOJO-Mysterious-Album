...
async function decision(target='question'){
  for(let i=0;i<200;i++){const s=await state();if(s.phase===target&&!s.busy)return;if(await page.locator('#speech-card').isVisible())await page.locator('#speech-card').click({timeout:1500});await page.waitForTimeout(70);}
...
async function meet(){
  ...
  checks.push(`${before.storyId||'debate'}: walk up, dialogue, response, movement pause, main progress unchanged`);
}
try{
  ...
  checks.push('First question carried to debate; three topics; offline authored discussion; child idea carried to chapter three');
  await meet();
  ...
  checks.push('Compound blue creation, requested helper, modify existing object, visible world and storage restored on refresh');
  ...
  await page.screenshot({path:`${out}/moon-workshop.png`});checks.push('All six creative scenes, explicit onward action, ending and continuing to create');
  ...
  checks.push('One visible instance per friend; backpack returns to a previous planet and its saved work');
  ...
  checks.push('Three mobile entries, no horizontal overflow or removed navigation controls');assert.deepEqual(errors,[]);
  const report={passed:true,base,checks,errors,note:'Deterministic offline API and speech fallback; no live microphone capture claimed.'};await writeFile(`${out}/report.json`,JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
