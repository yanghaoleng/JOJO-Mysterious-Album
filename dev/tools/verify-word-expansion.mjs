...
for(const text of ['Two blue jellyfish.','Three little starfish.']){const s=createWordSuggestions({mode:'open',example:text});const index=s.parts.find(p=>/^(jellyfish|starfish)$/.test(p.text)).index;s.change(index);assert.match(s.text,/lanterns|mooncakes|rabbits|turtles|octopuses|jellyfish|starfish|fishes|tents|acorns|pinecones|hedgehogs|penguins|seals|walruses|igloos|hands|feet|eyes|ears|flowers|robots|balls|boxes|cars|ducks|birds|elephants|chicks|chickens|giraffes|zebras|snakes|oxen|tigers|dragons|horses|goats|monkeys|roosters/);}
