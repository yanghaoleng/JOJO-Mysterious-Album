...
for(const color of ['粉色','彩虹色'])for(const object of ['棉花糖','小鱼']){
 const entries=[{id:'gugu-feeling',answer:`${color}天空里有${object}`}];
 const commands=firstLightWorldCommands(story,story.scenes[4],entries);
...
assert.equal(firstLightModelChoice(story.scenes[2],'七彩天空里有小鱼'),'彩虹色天空里有小鱼');
assert.equal(firstLightModelChoice(story.scenes[2],'粉色天空里有小鱼'),'粉色天空里有小鱼');
...
try{
 ...
 const entries=story.scenes.slice(0,2).map(s=>({id:s.id,chapter:1,kind:s.wow.kind,answer:s.id==='voice-light'?'为什么天是蓝的？':s.choices[0].label,reaction:'啵！',source:'local'}));
 await page.addInitScript(entries=>{if(sessionStorage.getItem('fl-fixture'))return;sessionStorage.setItem('fl-fixture','1');localStorage.setItem('jma.dev.clay.v1.story.wow',JSON.stringify({sceneIndex:2,sceneId:'gugu-feeling',scriptVersion:4,setupDone:true,inventory:[{id:'torch'}],inventions:[],wowEntries:entries,firstWords:'为什么天是蓝的？'}));},entries);
 ...
 await page.locator('#start-story').click();await decision();await choose('把光种进草地');
 saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('jma.dev.clay.v1.story.wow')));assert.equal(saved.worldState.worlds.meadow.entities['fl-trail'].asset,'prop:light-trail');
 ...
 console.log('PASS: real choice buttons compose rainbow and fish, mobile, refresh, sprout and independent gallery preview');
