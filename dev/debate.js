...
async function answer(raw){const text=String(raw).trim().slice(0,phase==='reflection'?120:80);if(!text||document.body.dataset.encounter||!['ready','topic','reflection'].includes(phase))return;if(isVagueAnswer(text)){showChoices(phase==='reflection');status('慢慢想，也可以选一个想法。');return;}if(/\d{7,}|身份证|住在|学校叫|手机号|自杀|杀人|炸弹|色情/.test(text)){status('个人信息不用告诉我们。选一个关于好奇心的问题吧。');return;}input.setTranscript(text);$('answer-input').value='';$('answer-dialog').close();voice.listen(false);cancelSpeech();status('');
  ...
  try{const data=await requestJSON('/api/debate',{question:text,speakers:speakers.map(({id,name,hint})=>({id,name,hint}))},45000,request.signal);if(token!==epoch)return;if(data.allowed===false){setPhase('topic');$('speech-text').textContent=data.safeMessage;status(data.safeMessage);return;}if(!Array.isArray(data.turns)||data.turns.length!==4)throw new Error('Invalid debate response');result=data;index=0;paused=false;$('toggle-play').textContent='暂停';$('topic-caption').textContent=data.topic;status('');setPhase('discussing');void playTurn();}
