...
const answerSupport = new AnswerSupport({ available: () => phase === 'reflection' && !speech.isActive() && !$('answer-dialog').open && !document.hidden && $('answer-choices').hidden, reveal: () => showChoices(true) });
...
function showChoices(reflection = false) {
  const choices = reflection ? ['我想先试试小鸮的办法。', '我想先试试小兔的办法。', '我想把两个办法合起来试试。']
    : ['星球上可以只种棒棒糖吗？', '开船时大家可以轮流当船长吗？', '去月球旅行，要先计划好还是边走边想？'];
...
async function answer(raw){const text=String(raw).trim().slice(0,phase==='reflection'?120:80);if(!text||!['ready','topic','reflection'].includes(phase))return;if(isVagueAnswer(text)){status('慢慢想，也可以选一个想法。');return;}input.setTranscript(text);$('answer-input').value='';$('answer-dialog').close();voice.listen(false);cancelSpeech();status('');
  ...
  catch(error){if(token!==epoch)return;setPhase('topic');$('speech-text').textContent='刚才的想法没能送到。我们可以再试一次。';status('网络暂时没连上，请再选一个话题试试。');}finally{if(token===epoch)request=null;}
}
