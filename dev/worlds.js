...
export function createWorld(requestedId, { seed = 1 } = {}) {
  ...
  const clay = (color, emissive = false, transparent = false, surface = emissive ? 'ink' : transparent ? 'paint' : 'paper') => {
    const key = `${color}/${emissive}/${transparent}/${surface}`;
    if (!materials.has(key)) materials.set(key, new THREE.MeshStandardMaterial({
      color, roughness: 0.84, metalness: 0,
      userData: { handcraftedSurface: surface },
  ...
  const paper = color => clay(color, false, false, 'paper');
  const wood = color => clay(color, false, false, 'wood');
  const stone = color => clay(color, false, false, 'stone');
  const foliage = color => clay(color, false, false, 'foliage');
  const fabric = color => clay(color, false, false, 'fabric');
  const paint = color => clay(color, false, false, 'paint');
  const water = color => clay(color, false, false, 'water');
  const ink = color => clay(color, false, false, 'ink');
  const mesh = (geo, color, pos = [0, 0, 0], scale = [1, 1, 1], parent = staticRoot, rotation) => {
    const item = new THREE.Mesh(geo, typeof color === 'string' ? clay(color, false, false, parent.userData.handcraftedSurface || 'paper') : color);
  ...
  const tree = (x, z, height = 2.9, fruit = false, color = '#7f9f65') => {
    ...
    [[0, 0.81, 0, 0.82], [-0.62, 0.7, 0.04, 0.61], [0.61, 0.77, -0.1, 0.62], [-0.22, 0.93, -0.13, 0.57]].forEach(([dx, dy, dz, r]) => ball(foliage(color), dx, height * dy, dz, r, r * 0.88, r * 0.83, node));
    if (fruit) [[-0.51, 0.68, 0.54], [0.43, 0.75, 0.48], [0.05, 0.87, 0.59], [-0.2, 0.68, -0.54]].forEach(([dx, dy, dz]) => apple(dx, height * dy, dz, 0.16, node));
    ball(foliage('#b2c58b'), -0.13, 0.09, 0, 0.57, 0.12, 0.48, node);
    return node;
  };
  ...
  const fence = (x, z, width, rotation = 0) => {
    const node = part(x, 0, z, staticRoot, rotation, 'wood');
  ...
  const path = (points, color = '#e4d3ab', size = 0.55) => points.forEach(([x, z], i) => {
    const pavingStone = ball(stone(color), x, 0.035, z, size * (0.9 + (i % 2) * 0.2), 0.055, size * 0.6);
    pavingStone.rotation.y = i * 0.8;
  });
  const bone = (x, y, z, scale, parent) => {
    box(paint('#f0e2bb'), x, y, z, scale * 0.72, scale * 0.25, scale * 0.15, parent);
    [-1, 1].forEach(s => [-1, 1].forEach(t => ball(paint('#f0e2bb'), x + s * scale * 0.4, y + t * scale * 0.12, z, scale * 0.17, scale * 0.17, scale * 0.1, parent)));
  };
  const windowCircle = (x, y, z, parent, size = 0.26) => {
    cylinder(clay('#edc881', true, false, 'paint'), x, y, z, size, size, 0.06, parent, [Math.PI / 2, 0, 0]);
    const frameSurface = parent.userData.handcraftedSurface === 'paint' ? paint : wood;
    torus(frameSurface('#a58361'), x, y, z + 0.04, size, 0.045, parent, [0, 0, 0]);
    box(frameSurface('#ac885f'), x, y, z + 0.08, size * 1.85, 0.055, 0.04, parent);
    box(frameSurface('#ac885f'), x, y, z + 0.08, 0.055, size * 1.85, 0.04, parent);
  };
  const cottage = (x, z, scale = 1, roofColor = '#be6a55', sign = 'bone', rotation = 0) => {
    ...
    box(paint(roofColor), -0.57, 2.29, 0, 1.55, 0.16, 1.95, node, [0, 0, Math.PI / 4.1]);
    box(paint(roofColor), 0.57, 2.29, 0, 1.55, 0.16, 1.95, node, [0, 0, -Math.PI / 4.1]);
    for (let i = 0; i < 7; i++) rod('#d09a7b', [-1.07, 1.79, -0.81 + i * 0.27], [0, 2.79, -0.81 + i * 0.27], 0.027, node);
    for (let i = 0; i < 7; i++) rod('#d09a7b', [1.07, 1.79, -0.81 + i * 0.27], [0, 2.79, -0.81 + i * 0.27], 0.027, node);
    box(wood('#93765e'), 0, 0.58, 0.817, 0.66, 1.15, 0.07, node);
    arc(wood('#af9070'), 0, 1.14, 0.84, 0.35, 0.05, node);
    cylinder(wood('#93765e'), 0, 1.14, 0.82, 0.32, 0.32, 0.04, node, [Math.PI / 2, 0, 0]);
    ball(paint('#d9b76c'), 0.2, 0.61, 0.89, 0.045, 0.045, 0.032, node);
    windowCircle(-0.65, 1.15, 0.84, node, 0.21);
    windowCircle(0.65, 1.15, 0.84, node, 0.21);
    box(stone('#b49370'), 0.62, 2.45, -0.37, 0.3, 0.68, 0.33, node);
    box(stone('#ceb08a'), 0.62, 2.81, -0.37, 0.42, 0.12, 0.44, node);
    box(stone('#dec39b'), 0, 0.07, 0.99, 0.93, 0.14, 0.48, node);
  ...
  const globeMaterial = new THREE.MeshStandardMaterial({ color: '#ffffff', vertexColors: true, roughness: 0.94, userData: { handcraftedSurface: id === 'pocket' ? 'fabric' : ['moon', 'observatory', 'reef'].includes(id) ? 'stone' : 'paper' } });
  ...
  if (id === 'orchard') {
    ...
    const basket = part(2.83, 0.03, 0.62, staticRoot, 0, 'wood');
  ...
  if (id === 'bakery') {
    ...
    const awning = part(0, 1.52, 0.87, bakery, 0, 'fabric');
    for (let i = 0; i < 6; i++) box(i % 2 ? '#f0dcad' : '#b97357', -0.82 + i * 0.33, 0, 0.27, 0.33, 0.085, 0.75, awning, [0.13, 0, 0]);
    const oven = part(2.55, 0, -1.69, staticRoot, 0, 'stone');
    ...
    const table = part(2.7, 0, 0.95, staticRoot, -0.2, 'wood');
    ...
    box(paper('#f1e5cb'), 0, 0.02, 0.06, 0.75, 0.87, 0.018, sketch);
    rod('#ad9270', [-0.32, -0.68, -0.06], [-0.22, 0.32, -0.06], 0.035, sketch);
    rod('#ad9270', [0.32, -0.68, -0.06], [0.22, 0.32, -0.06], 0.035, sketch);
    shapeMesh([[-0.25, 0.2], [0, 0.4], [0.25, 0.2]], ink('#b87762'), 0.015, sketch, [0, 0, 0.076], 0.008);
    box(ink('#c6b290'), 0, 0.13, 0.082, 0.39, 0.14, 0.012, sketch);
    bone(0, -0.09, 0.089, 0.29, sketch);
    rod(ink('#a89477'), [-0.27, -0.28, 0.085], [0.27, -0.28, 0.085], 0.018, sketch);
    [-0.24, 0, 0.24].forEach(x => rod(ink('#a89477'), [x, -0.35, 0.085], [x, -0.2, 0.085], 0.015, sketch));
  ...
  if (id === 'bridge' || id === 'cove') {
    ...
    if (id === 'bridge') {
      const bridge = part(0.4, 0.04, 1.82, liveRoot, -0.18, 'wood');
      for (let i = 0; i < 9; i++) {
        const x = (i - 4) * 0.32;
      ...
      const boat = part(-2.3, 0.075, 1.98, liveRoot, 0.25, 'wood');
      ...
      const shell = part(1.5, 0.16, -2.45, liveRoot, -0.24, 'stone');
      for (let i = 0; i < 9; i++) {
        const a = -0.95 + i * 0.24;
  ...
  if (id === 'home') {
    ...
    const sign = part(-1.37, 0, 1.83, staticRoot, 0, 'wood');
  ...
  if (id === 'observatory') {
    ball(stone('#829796'), -2.48, 0.13, -1.82, 1.49, 0.24, 1.3);
    const telescope = part(-2.5, 0.23, -1.83, liveRoot, -0.42, 'wood');
    ...
    const tube = part(0, 1.4, 0, telescope, 0, 'paint'); tube.rotation.x = -0.66;
    ...
    cylinder(stone('#909fa7'), 1.88, 0.045, -1.74, 1.28, 1.33, 0.09);
    torus(paint('#e3d4a9'), 1.88, 0.105, -1.74, 1.1, 0.045);
    const rocket = part(1.88, 0.18, -1.74, liveRoot, 0, 'paint');
  ...
  if (id === 'reef') {
    ...
    const chest = part(-2.74, 0.15, 0.59, staticRoot, 0.2, 'wood');
    box('#b18b62', 0, 0.32, 0, 1.03, 0.57, 0.68, chest);
    cylinder('#9b7858', 0, 0.62, 0, 0.34, 0.34, 1.04, chest, [0, 0, Math.PI / 2]);
    [-0.33, 0.33].forEach(x => box(paint('#d0b573'), x, 0.36, 0.355, 0.09, 0.73, 0.04, chest));
    box(paint('#d6ba75'), 0, 0.51, 0.38, 0.13, 0.19, 0.05, chest);
    const bolt = part(2.62, 0.07, 0.69, liveRoot, 0.3, 'paint');
  ...
  if (id === 'pocket') {
    ...
    mesh(clothGeo, fabric('#9889ac'), [0, -0.05, -3.02]);
    rod(fabric('#cbb7c9'), [-3.13, 3.32, -2.78], [0, 3.08, -2.78], 0.105);
    rod(fabric('#cbb7c9'), [0, 3.08, -2.78], [3.13, 3.32, -2.78], 0.105);
    for (let i = 0; i < 21; i++) {
      const x = -3.15 + i * 0.31;
      rod(fabric('#dec9d1'), [x, 2.9 + Math.abs(x) * 0.064, -2.72], [x + 0.11, 2.96 + Math.abs(x) * 0.064, -2.72], 0.028);
    }
    for (let i = 0; i < 6; i++) [-1, 1].forEach(side => rod(fabric('#cfbbc9'), [side * 3.49, 0.32 + i * 0.36, -2.68], [side * 3.5, 0.48 + i * 0.36, -2.68], 0.028));
    const spool = part(-2.65, 0, -0.82, staticRoot, 0.18, 'wood');
    cylinder('#cfb596', 0, 0.71, 0, 0.65, 0.65, 1.29, spool);
    [0.13, 1.36].forEach(y => cylinder('#ba9b78', 0, y, 0, 0.8, 0.8, 0.15, spool));
    for (let i = 0; i < 10; i++) torus(fabric('#dfc09e'), 0, 0.25 + i * 0.1, 0, 0.651, 0.025, spool);
    const button = (x, y, z, radius, color, parent = staticRoot, rotation = [-Math.PI / 2, 0, 0]) => {
      const node = part(x, y, z, parent, 0, 'paint'); node.rotation.set(...rotation);
      cylinder(color, 0, 0, 0, radius, radius, 0.15, node, [Math.PI / 2, 0, 0]);
      torus('#ddcbbb', 0, 0, 0.09, radius * 0.79, 0.025, node, [0, 0, 0]);
      [-1, 1].forEach(s => [-1, 1].forEach(t => ball(ink('#756c7e'), s * radius * 0.2, t * radius * 0.2, 0.082, radius * 0.07, radius * 0.07, 0.026, node)));
      return node;
    };
    button(2.62, 0.72, -1.72, 0.72, '#cda0a2', staticRoot, [0.07, -0.25, -0.12]);
    button(2.85, 0.14, 1.16, 0.49, '#d9c58d');
    const thread = part(-2.16, 0.2, 1.52, liveRoot, 0, 'fabric');
    ...
    const ladder = part(0.89, 0, -2.12, liveRoot, 0, 'wood'); ladder.rotation.x = -0.2;
  ...
  if (id === 'cloud') {
    ...
    const gate = part(0.25, 0.06, -2.78, staticRoot, 0, 'paint');
    ...
    const compass = part(2.84, 0.03, 0.55, liveRoot, 0, 'paint');
  ...
  if (id === 'moon') {
    const crater = (x, z, radius) => {
      cylinder(stone('#a6aebd'), x, 0.007, z, radius * 0.93, radius * 0.93, 0.025);
      torus(stone('#cdd0d8'), x, 0.095, z, radius, radius * 0.12);
      torus(stone('#b2b8c5'), x, 0.049, z, radius * 0.82, radius * 0.032);
    };
    crater(-2.1, -2.12, 1.22); crater(2.92, 1.24, 0.57); crater(-2.87, 1.9, 0.42);
    const listeningShell = part(-2.08, 0.17, -0.63, staticRoot, 0, 'stone');
    for (let i = 0; i < 6; i++) {
      const a = -0.9 + i * 0.36;
    ...
    const flagpole = part(1.87, 0, -2.2, staticRoot, 0, 'wood');
    ...
    for (let i = 0; i < 7; i++) {
      const z = -1.1 + i * 0.47;
      [-0.1, 0.1].forEach((offset, n) => ball(stone('#aab2c2'), 0.67 + Math.sin(i * 0.6) * 0.19 + offset, 0.012, z + n * 0.1, 0.05, 0.015, 0.092));
    }
  }
  ...
  if (id === 'meadow') {
    ...
    const log = part(-2.5, 0.28, 0.63, staticRoot, 0.17, 'wood');
  ...
  for (let i = 0; i < 14; i++) {
    ...
    if (id === 'moon' || id === 'observatory') {
      ...
      [-1, 1].forEach(x => [-1, 1].forEach(z => ball(ink('#8a7897'), x * size * 0.15, 0.14, z * size * 0.15, 0.035, 0.012, 0.035, anchor)));
    } else if (id === 'reef') {
      ...
      if (i % 4 === 0 && ['orchard', 'meadow', 'home', 'bakery'].includes(id)) {
        rod(wood('#a28562'), [0, 0.05, 0], [0, 0.65, 0], 0.075, anchor);
  ...
  if (id === 'pocket') {
    for (let i = 0; i < 48; i++) {
      ...
      rod(fabric('#dfcdd4'), threadPoint(angle - 0.032), threadPoint(angle + 0.032), 0.027, backsideRoot);
    }
  }
