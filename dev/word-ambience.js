...
export function createWordAmbience({entities,send,occupied=()=>[],canAct=()=>true,reduced=()=>false,camera=()=>{},random=Math.random,now=Date.now}) {
  ...
  function tick(){if(disposed)return;const at=now();if(lease&&at>=lease.until)release();if(!canAct())return;
    ...
    const actors=all.filter(e=>free(e)&&(/^(npc:|yellow:|actor:)/.test(e.asset)||/^prop:(rword-)?(cat|dog|duck|bird|hen|pig|bear|rabbit|bunny|frog|fox|rat|cub|teddy|robot)$/.test(e.asset)));
    if(!actors.length)return;const actor=actors[Math.floor(random()*actors.length)],foods=all.filter(e=>free(e)&&/^prop:(rword-)?(apple|nut|cake|bread|banana|carrot|cookie|hamburger)$/.test(e.asset));
