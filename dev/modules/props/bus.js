// 公交车: independent prefab using the shared material and geometry toolkit.
export function build({THREE,group,part,moving,beam,color,cream,wood,gold,ink}) {
  ...
  part("box",color,[0,.85,0],[2.4,1.1,.9]);for(let i=0;i<4;i++)part("box",cream,[-.85+i*.55,1,.46],[.4,.4,.02]);wheels();
