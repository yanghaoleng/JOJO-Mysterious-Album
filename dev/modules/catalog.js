...
export const COMPONENTS = [
  ...
  {id:'story:words',kind:'story',name:'独立章节 · 开口造世界',description:'选具体年龄后只推荐一章，九章各六轮，中文引出单个名词、第二个名词、数量、颜色、大小与最后一句话；中秋独立章节直达，扩充到 54 个节日词。实际入口 /dev/words.html。',source:'dev/content/word-games.js',capabilities:['三个年龄档','九章162轮与中秋独立章节','中秋54个词与十款新造型','中文问答与渐进短语','第三步彩虹换词教学','目标词与知识点','开放例句','海底、露营与冰雪主题','12款扩展名词模型','13款常见动物与完整十二生肖'],dependencies:['logic:word-progress','logic:word-intent']},
  ...
  {id:'logic:word-realtime-voice',kind:'logic',name:'英语冒险 · 可切换实时语音',description:'在单词菜单切换豆包端到端实时对话与经典语音。新服务需开通 volc.speech.dialog 权限；连接失败回退经典方案。连续音频播放工作线程消除流式分段接缝，验证 test_word_realtime.py 与 verify-pcm-playback.mjs。',source:'dev/word-realtime-voice.js',capabilities:['实时音频与字幕','连续 PCM 音频与缓冲抗抖','英文角色道具热词','经典语音回滚','断线回退','Domi专属童声','更慢的课堂女声与逐词停顿'],dependencies:['ui:voice']},
  ...
  {id:'logic:word-effects',kind:'logic',name:'所有物件的生长与动作',description:'花和便便都能生长、跳、飞、游、变长或表达心情；点击便便会飞走并清理，明亮等词使物件发光。验证 dev/tools/verify-word-game.mjs 与 verify-poop-flight-ui.mjs。',source:'dev/presentation/word-effects.js',capabilities:['点名跳两下与落入盒子','生长与缩小','跳跃飞行游泳','点击便便飞走','节日词发光','模型表面笑脸','情绪','部件装配','存档恢复'],dependencies:['logic:presenter']},
