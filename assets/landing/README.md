...
`story-wow`, `story-debate`, and `story-moon` are screenshots of the actual playable 3D story pages. `tools/capture-story-covers.mjs` advances the first chapter until Gugu appears and hides only the interface while capturing the scene. `hero-immersive` and `hero-immersive-mobile` are static fallbacks captured from the homepage renderer.
