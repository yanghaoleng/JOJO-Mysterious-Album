...
import { createLandingSky } from './landing-sky.js?v=20260918-living';
...
export function createLandingScene(container,{motion=true}={}) {
  ...
  const sky=createLandingSky(stage);
  ...
  let oldScenery=-1,actorTransition=null,lastPaint=-1;
  ...
  function paintPlanet(force=false){
    if(!force&&elapsed-lastPaint<.075)return;lastPaint=elapsed;
    const t=reduced?1:ease((elapsed-schedules.planet.at)/2.5);
  ...
  function sceneryVisibility(index,amount){
    scenery[index].forEach(root=>{root.visible=amount>.001;root.position.y=(1-amount)*-.65;});
  ...
  stage.renderer.setAnimationLoop(now=>{
    ...
    if(actorTransition&&!actorTransition.swapped&&elapsed-actorTransition.start>.35){
    ...
    const entrance=reduced?1:ease(elapsed/1.5);
    globe.position.y=-4.5-(1-entrance)*2.3;
    globe.scale.setScalar(.94+.06*entrance);
    const sceneryEnter=reduced?1:ease((elapsed-propTrack.at-(propTrack.index?0:.4))/1.15);
    worlds.forEach((world,i)=>{
      const amount=i===propTrack.index%worlds.length?sceneryEnter:i===oldScenery?1-sceneryEnter:0;
      sceneryVisibility(i,amount);if(amount>0)world.update(reduced?0:elapsed,reduced?0:dt);
    });
    for(const actor of stage.actors.values()){
      actor.update(reduced?0:elapsed,reduced?0:dt);
      let amount=reduced?1:ease((elapsed-.75)/.9);
      if(actorTransition){const age=elapsed-actorTransition.start;amount=age<.35?1-ease(age/.35):ease((age-.35)/.7);if(age>=1.05)actorTransition=null;}
      actor.group.scale.setScalar(.78*Math.max(.001,amount));actor.surfaceGrounding?.update();
      actor.group.position.y+=(1-amount)*1.3;
    }
  ...
  container.__heroScene={get status(){return {time:elapsed,reduced,visible,actor:schedules.actor.index,scenery:schedules.scenery.index,planet:schedules.planet.index,next:{actor:schedules.actor.next,scenery:schedules.scenery.next,planet:schedules.planet.next},sky:sky.stats,drawCalls:stage.renderer.info.render.calls,geometries:stage.renderer.info.memory.geometries};}};
  return {renderer:stage.renderer,stage,dispose(){disposed=true;observer.disconnect();preference.removeEventListener('change',onPreference);sky.dispose();worlds.slice(1).forEach(world=>world.dispose());delete container.__heroScene;stage.dispose();}};
}
