...
function openJourney(id){
  ...
  const entry=(step,i)=>`<article class="journey-moment journal-entry">
    ...
    </div><figure class="journey-effect"><img src="./assets/landing/journey-${id}-${i+1}.webp" width="1200" height="900" alt="${escapeHTML(step.effect)}" loading="lazy"><figcaption><span>${icon('sparkles')} ${j.child}的想法，让世界变了</span><p>${escapeHTML(step.effect)}</p></figcaption></figure></div>
  </article>`;
  ...
  show(`<div class="journey-journal journal-${j.theme}">
    <header class="journal-header"><div class="journal-identity"><span class="preview-label">示例冒险日志</span><div class="journal-name">${avatar()}<div><span class="journal-owner">这一次，由我来想象</span><h2 id="preview-title">${j.child}的冒险日志</h2><p>${j.title}</p></div></div><blockquote>“${escapeHTML(j.wish)}”</blockquote><p class="journal-route-caption">${icon('footprints')}${escapeHTML(j.route)}</p></div>
    ...
    <section class="journal-keepsakes" aria-labelledby="keepsake-title"><div class="journal-section-title"><h3 id="keepsake-title">${j.child}的发明口袋</h3><span>东西，也可以是一条新规则。</span></div><div class="journal-inventions">${j.steps.map((step,i)=>`<div class="journal-invention"><img src="./assets/landing/journey-${id}-${i+1}.webp" width="1200" height="900" alt="" loading="lazy"><div><span>${icon(step.icon)}发明 ${String(i+1).padStart(2,'0')}</span><h4>${escapeHTML(step.invention)}</h4></div></div>`).join('')}</div></section>
